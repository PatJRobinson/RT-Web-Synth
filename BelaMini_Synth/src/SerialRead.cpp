...
void SerialRead::setup (InputData * inStruct, const char dev[], int bd)
{
    //_midi = m;
...
int SerialRead::setupSerialDevice (const char dev[], int bd)
{
    /*
...
void SerialRead::readSerialDevice()
{
    midi_byte_t buf[3];
    ...
    while (!gShouldStop)
    {
        ...
        if (_serial_fd.revents & POLLIN)
        {
            ...
            */
            ...
            inStruct_->write(buf_);
            //int in = inStruct_->getVal(159, 15);
			inStruct_->print(buf_[0], buf_[1]);
